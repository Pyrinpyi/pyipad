...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-pyipad.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "pyipad")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of pyipad binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-pyipad.conf")
